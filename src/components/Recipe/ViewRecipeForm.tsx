...
const ViewRecipeForm = () => {
    ...
    const onSubRecipeSelect = async (subRecipeId: number) => {
        ...
        const formData: Omit<IRecipeSubRecipe, 'id'> = {
            parent_recipe_id: recipeIdNumber,
            sub_recipe_id: subRecipeId,  // the value of the clicked picker item
            quantity: 0,
    ...
    const onIngredientSelect = async (ingredientId: number) => {
        const recipeIdNumber = Number(id);
        if (!recipeIdNumber || auth.isLoading || !auth.user?.access_token) {
            showFlashMessage('You must be logged in to modify a recipe', 'danger');
            return;
        ...
        const formData: Omit<IRecipeIngredient, 'id'> = {
    ...
    const onLabourSelect = async (labourerId: number) => {
        const recipeIdNumber = Number(id);
        if (!recipeIdNumber || auth.isLoading || !auth.user?.access_token) {
            showFlashMessage('You must be logged in to modify a recipe', 'danger');
            return;
        ...
        const formData: Omit<IRecipeLabour, 'id'> = {
            recipe_id: recipeIdNumber,
